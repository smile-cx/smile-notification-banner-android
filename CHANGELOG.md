...
### Fixed
- Removed unused `context` parameter from `schedulePending()` method
  - **Before**: `SmileBanner.schedulePending(context)`
  - **After**: `SmileBanner.schedulePending()`
  - Context was never used in the implementation, simplifying the API
  - This is a minor breaking change for users who upgraded to 2.5.0

### Changed
- Updated all tests and sample app to use simplified API
- Updated documentation to reflect parameter removal
